...
if (vueSalleFiltre === undefined) {
  class VueSalleFiltre {
    ...
    init(): void {
      this.idSelect = "";
      (<HTMLDivElement>document.querySelector("[id=filtre_recherche]")).hidden =
        true;
      // aucun filtre déployé
      this.aucuneSelection(); // aucun filtre sélectionné
    }
    itemClick(event: Event): void {
      const cible = <HTMLDivElement>event.target; // clic dans la div "filtre_choix
      ...
      if (cible.getAttribute("name") === "bouton") {
        ...
        const divRecherche = <HTMLDivElement>(
          document.querySelector("[id=filtre_recherche]")
        );
    ...
    aucuneSelection(): void {
      // lister les div 'bouton' du filtre choix
      const choix = <HTMLDivElement>document.querySelector("[id=filtre_choix]");
      const lesBoutons: NodeListOf<HTMLDivElement> =
        choix.querySelectorAll("[name=bouton]");
      for (let bouton of lesBoutons) {
        ...
        (<HTMLDivElement>bouton.nextElementSibling).style.display = "none";
        bouton.classList.remove("pnlselection");
        // réinitialiser (vider) toutes les zones de saisie du filtre
        const lesInputs: NodeListOf<HTMLInputElement> =
          choix.querySelectorAll("input");
    ...
    rechercherClick(): void {
      let chaine = "";
      // récupération des zones de saisie (input) du filtre sélectionné
      const choix = <HTMLDivElement>(
        document.querySelector("[id=" + this.idSelect + "_edit]")
      );
      const lesInputs: NodeListOf<HTMLInputElement> =
        choix.querySelectorAll("input");
...
(<HTMLElement>document.querySelector("[id=filtre_choix]")).addEventListener(
  "click",
  function (event) {
    vueSalleFiltre.itemClick(event);
  }
);
(<HTMLElement>document.querySelector("[id=btn_rechercher]")).addEventListener(
  "click",
  function () {
