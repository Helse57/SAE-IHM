type TvaleurStatut = "correct" | "vide" | "inconnu" | "doublon";
// les erreurs "champ vide", "valeur inconnue", "doublon" sont les trois principales erreurs
// dans un formulaire. Chaîne "correct" signifie pas d'erreur
type TtabErreur = {
  [key: string]: TvaleurStatut;
  // tableau contenant le type d'erreur ou pas, constaté pour chaque zone de saisie à vérifier
};
type TerreurStatut = {
  // message d'erreur pour un type d'erreur constaté
  [key in TvaleurStatut]: string;
};
type TtabMsgErreur = {
  // tableau contenant les messages d'erreur pour chaque type d'erreur pour chaque zone de saisie à vérifier
  [key: string]: TerreurStatut;
};
type Tinput = { [key: string]: HTMLInputElement }; // stockage « input » du formulaire
// test pour ne définir que la classe et l'objet une seule fois sinon redéclaration ==> erreur
if (vueSalleEdit === undefined) {
  class VueSalleEdit {
    private _lesSalles: LesSalles;
    private _salle: UneSalle;
    private _params: Tparams;
    private _erreur: TtabErreur;
    private _msgErreur: TtabMsgErreur;
    ...
    init(params: Tparams): void {
      ...
      (<HTMLInputElement>(
        document.querySelector("[id=salle_titre]")
      )).textContent = titre;
      if (this._params.statut !== "ajout") {
        // affi ou modif
      ...
      (<HTMLInputElement>(
        document.querySelector("[id=salle_btn_retour]")
      )).hidden = this._params.statut !== "affi";
      (<HTMLInputElement>(
        document.querySelector("[id=salle_btn_valider]")
      )).hidden = this._params.statut === "affi";
      (<HTMLInputElement>(
        document.querySelector("[id=salle_btn_annuler]")
      )).hidden = this._params.statut === "affi";
      let chaine: string = "?" + this._params.statut + "&" + this._params.id;
    ...
    detailDepartement(valeur: string): void {
      ...
      const detail = <HTMLDivElement>(
        document.querySelector("[id=salle_departement]")
      );
      detail.textContent = "";
      ...
      } else this._erreur["erreur_dept"] = "vide";
    }
    retourClick(): void {
    ...
    validerClick(): void {
      ...
      } else this._erreur["erreur_equipt"] = "correct";
      ...
      for (let id in this._erreur) {
        ...
        if (this._erreur[id] !== "correct") {
          ...
          if (this._msgErreur[id][this._erreur[id]] != "") {
            ...
            (<HTMLSpanElement>(
              document.querySelector("[id=" + id + "]")
            )).textContent = this._msgErreur[id][this._erreur[id]];
            correct = false;
          }
        }
      }
      ...
      if (correct) {
        ...
        const lesTypEquiptsBySalle: LesTypEquiptsBySalle =
          new LesTypEquiptsBySalle();
        lesTypEquiptsBySalle.delete(this._salle.numSalle);
        lesTypEquiptsBySalle.insert(
          this._salle.numSalle,
          vueSalleEquipement.data
        );
        this.retourClick();
      }
    }
...
(<HTMLInputElement>(
  document.querySelector("[id=salle_edt_codedept]")
)).addEventListener("change", function () {
  vueSalleEdit.detailDepartement((<HTMLInputElement>this).value);
});
