...
if (vueSalleListe === undefined) {
    class VueSalleListe {
        ...
        init(params) {
            ...
            switch (this._params.id) {
                ...
                case "filtre_departement":
                    this._data = this._lesSalles.listByDepartement(this._params.elts[0], this._params.elts[1]);
                    break;
            }
            this._grille = APIpageWeb.showArray("salle_table", this._data, "numSalle", true);
        ...
        supprimerClick() {
            if (this._grille.getIdSelect() !== "") {
                // est-ce qu'une ligne est sélectionnée dans le tableau ?
                if (confirm("Confirmez-vous la suppression de l'infraction " +
...
(document.querySelector("[id=salle_btn_detail]")).addEventListener("click", function () {
    vueSalleListe.afficherClick();
});
(document.querySelector("[id=salle_btn_ajouter]")).addEventListener("click", function () {
    vueSalleListe.ajouterClick();
});
(document.querySelector("[id=salle_btn_modifier]")).addEventListener("click", function () {
    vueSalleListe.modifierClick();
});
(document.querySelector("[id=salle_btn_supprimer]")).addEventListener("click", function () {
    vueSalleListe.supprimerClick();
});
//# sourceMappingURL=salle_liste.js.map